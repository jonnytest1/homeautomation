...
int16_t LoRaWANNode::restore()
{
  ...
  Serial.println(F("channel restore OTAA"));
...
int16_t LoRaWANNode::restoreChannels()
{
  ...
  if (this->band->bandType == RADIOLIB_LORAWAN_BAND_DYNAMIC)
  {
    ...
    for (int i = 0; i < RADIOLIB_LORAWAN_NUM_AVAILABLE_CHANNELS; i++)
    {
      ...
      { // only execute if it is not all zeroes
        cmd.repeat = 1;
...
int16_t LoRaWANNode::beginOTAA(uint64_t joinEUI, uint64_t devEUI, uint8_t *nwkKey, uint8_t *appKey, uint8_t joinDr, bool force)
{
  ...
  Serial.println("eeprom supported");
  if (validCheckSum && validMode)
  {
    ...
    {
      Serial.println("restoring dev nonce");
...
int16_t LoRaWANNode::beginABP(uint32_t addr, uint8_t *nwkSKey, uint8_t *appSKey, uint8_t *fNwkSIntKey, uint8_t *sNwkSIntKey, bool force)
{
  ...
  if (fNwkSIntKey)
  {
    checkSum ^= LoRaWANNode::checkSum16(fNwkSIntKey, 16);
  }
  if (sNwkSIntKey)
  {
    checkSum ^= LoRaWANNode::checkSum16(sNwkSIntKey, 16);
  }
    ...
    if ((this->fcntUp - this->adrFcnt) == (adrLimit + adrDelay))
    {
      ...
      while (adrStage != 0)
      {
        ...
        case (1):
        {
          ...
          if (adrStage == 1)
          { // if nothing succeeded, proceed to stage 2
            adrStage = 2;
          }
        }
        break;
        case (2):
        {
          ...
          if (adrStage == 2)
          { // if nothing succeeded, proceed to stage 3
            adrStage = 3;
          }
        }
        break;
...
int16_t LoRaWANNode::downlinkCommon()
{
  ...
  if (this->phyLayer->isRxTimeout())
  {
    ...
    Serial.println(F("default timeout"));
...
int16_t LoRaWANNode::selectChannels()
{
  ...
  for (uint8_t i = 0; i < RADIOLIB_LORAWAN_NUM_AVAILABLE_CHANNELS; i++)
  {
    if (this->availableChannels[RADIOLIB_LORAWAN_CHANNEL_DIR_UPLINK][i].enabled)
    {
      if (this->dataRates[RADIOLIB_LORAWAN_CHANNEL_DIR_UPLINK] >= this->availableChannels[RADIOLIB_LORAWAN_CHANNEL_DIR_UPLINK][i].drMin && this->dataRates[RADIOLIB_LORAWAN_CHANNEL_DIR_UPLINK] <= this->availableChannels[RADIOLIB_LORAWAN_CHANNEL_DIR_UPLINK][i].drMax)
      {
