...
public:
    ...
    */
    ...
    circular_queue(circular_queue &&cq) : m_bufSize(cq.m_bufSize), m_buffer(cq.m_buffer), m_inPos(cq.m_inPos.load()), m_outPos(cq.m_outPos.load())
    {
    }
    ~circular_queue()
    {
...
size_t circular_queue<T, ForEachArg>::push_n(const T *buffer, size_t size)
{
    ...
    size_t blockSize = (outPos > inPos) ? outPos - 1 - inPos : (outPos == 0) ? m_bufSize - 1 - inPos
                                                                             : m_bufSize - inPos;
...
T circular_queue<T, ForEachArg>::pop()
{
    ...
    if (m_inPos.load(std::memory_order_relaxed) == outPos)
        return defaultValue;
